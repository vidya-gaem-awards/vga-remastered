    ...
    public function calculateResults(): array
    {
        ...
        while ($currentRound < $roundsRequired - 1) {
            ...
            foreach ($firstPref as $candidate => $muhVotes) {
                $thisRound["Ranking"][] = $candidates[$candidate]->name . ": $muhVotes (".round($voteCount > 0 ? ($muhVotes/$voteCount*100) : 0, 2)."%)";
            }
        ...
        foreach ($firstPref as $candidate => $votes) {
            $thisRound["Ranking"][] = $candidates[$candidate]->name . ": $votes (".round($voteCount > 0 ? ($votes/$voteCount*100) : 0, 2)."%)";
        }
        $steps[] = $thisRound;

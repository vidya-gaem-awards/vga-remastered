...
class WikipediaService
{
    ...
    private const array PLATFORM_MAP = [
        ...
        'arcade' => ['Arcade', null],
        'atari vcs (2021 console)' => ['Atari VCS (2021)', null],
        'browser game' => ['PC', 'pc'],
        'dc' => ['DC', null],
        'dos' => ['DOS', 'pc'],
        'dreamcast' => ['DC', null],
    ...
    public function getGames(int $year): array
    {
        ...
        $result = Http::withUserAgent('Vidya Gaem Awards/1.0 (vidya@vidyagaemawards.com)')
            ->get($url)
            ->throw()
            ->json();
        ...
        if (empty($games)) {
            throw new Exception('Unable to parse the list of games for ' . $year . '. This may be due to formatting changes in the Wikipedia article.');
        }
    ...
    public function addGamesToGameReleaseTable(array $games, bool $deleteExisting = true): void
    {
        ...
        foreach ($games as $title => $platforms) {
            if ($doNotImport->contains($title)) {
                continue;
            }

            $gameRelease = new GameRelease();
            $gameRelease->list = 'video-games';
            $gameRelease->name = $title;
            $gameRelease->url = 'https://en.wikipedia.org/wiki/' . urlencode(str_replace(' ', '_', $title));
            $gameRelease->source = 'wikipedia';
            $gameRelease->platforms = $platforms;

            foreach ($platforms as $platform) {
                if (!isset(GameRelease::PLATFORMS[$platform])) {
                    Log::warning("Unknown platform for game $title: $platform");
                }
            }
